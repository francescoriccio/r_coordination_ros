...
namespace gmms {
  void GMMRegressor::train(const Eigen::MatrixXd &dataset, bool evaluate_bic, int gmm_components) {
    ...
    for (; num_components <= gmm_components; ++num_components) {
      std::cout << "\t\t\t\tUsing " << num_components << " GMM components" << std::endl;

      std::shared_ptr<GaussianMixtureModel> model(new GaussianMixtureModel);
      model->setNumComponents(num_components);
	...
	std::cout << "\t\t\t\t" <<  num_components << " components are not usable" << std::endl;
	break;
      }
